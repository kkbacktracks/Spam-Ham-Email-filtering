...
def get_final_matrix(spam_files, ham_files, all_files):
    ...
    for word in all_words_in_files:
        ...
        if word not in count_text_in_all_docs:
            if word not in stop_words and word!='Subject':
                count_text_in_all_docs[word] = 1
        else:
            if word not in stop_words and word!='Subject':
                count_text_in_all_docs[word] += 1
    ...
    for email in spam_files:
        ...
        spam_dict_all_docs = Counter(spam_dict_all_docs) + Counter(dictionary_temp)
    ...
    for email in ham_files:
        ...
        ham_dict_all_docs = Counter(ham_dict_all_docs) + Counter(dictionary_temp)
...
def model_bow_feature(dataset, isTrain):
    ...
    return model_bernoulli_spam_emails, model_bernoulli_ham_emails, count_text_in_all_docs,spam_dict_all_docs, ham_dict_all_docs, files_dict
    

#model_bernoulli_spam_emails, model_bernoulli_ham_emails, count_text_in_all_docs,spam_dict_all_docs, ham_dict_all_docs, files_dict = model_bow_feature('Datasets', True)
